...
public class PricingPlanRepository implements Serializable {
    ...
    public boolean savePricingPlan(PricingPlan pricingPlan) {
        try {
            ...
            String query = "INSERT INTO pricing_plans (id, name, price, billing_frequency, features, is_highlighted, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement stmt = connection.prepareStatement(query);
            ...
            stmt.setTimestamp(8, Timestamp.valueOf(pricingPlan.getCreatedAt()));
            stmt.setTimestamp(9, Timestamp.valueOf(pricingPlan.getUpdatedAt()));
    ...
    public boolean editPricingPlan(PricingPlan pricingPlan) {
        try {
            String update = "UPDATE pricing_plans SET name = ?, price = ?, billing_frequency = ?, features = ?, is_highlighted = ?, display_order = ?, updated_at = ? WHERE id = ?";
            PreparedStatement stmt = connection.prepareStatement(update);

            stmt.setString(1, pricingPlan.getName());
            stmt.setDouble(2, pricingPlan.getPrice());
            stmt.setString(3, pricingPlan.getBillingFrequency());
            stmt.setString(4, pricingPlan.getFeatures()); // Fixed: Add features
            stmt.setBoolean(5, pricingPlan.isHighlighted());
            stmt.setInt(6, pricingPlan.getDisplayOrder());
            stmt.setTimestamp(7, Timestamp.valueOf(pricingPlan.getUpdatedAt()));
            stmt.setString(8, pricingPlan.getId());
    ...
    public boolean updatePricingPlan(PricingPlan pricingPlan) {
        String query = "UPDATE pricing_plans SET name = ?, price = ?, billing_frequency = ?, features = ?, is_highlighted = ?, display_order = ?, updated_at = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, pricingPlan.getName());
            stmt.setDouble(2, pricingPlan.getPrice());
            stmt.setString(3, pricingPlan.getBillingFrequency());
            stmt.setString(4, pricingPlan.getFeatures());
            stmt.setBoolean(5, pricingPlan.isHighlighted());
            stmt.setInt(6, pricingPlan.getDisplayOrder());
            stmt.setTimestamp(7, Timestamp.valueOf(pricingPlan.getUpdatedAt()));
            stmt.setString(8, pricingPlan.getId());
